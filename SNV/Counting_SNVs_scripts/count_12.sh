...
echo "Part-0 1-based convert to 0-based"; date
...
echo "Part-1.1 Split 0-based VCF to 12 files"; date
mkdir -p ${workdir}/database/vcf12
cd ${workdir}/database/vcf12
awk ' {name=$5$7".bed"; print $0 >> name} ' OFS='\t' ${workdir}/database/vcf.bed
cd ${workdir}

echo "Part-1.2 For each subseted VCF file, sort."
for f in ${workdir}/database/vcf12/*.bed; do
...
mkdir -p ${workdir}/count/vcf12
cd ${workdir}/count/vcf12
for f in ${workdir}/database/vcf12/*.bed.sorted; do
    fb=$(basename $f) #AG.bed.sorted
...
Rscript merge_12.R \
    $workdir \
    ${run_prefix}_result.12.unstrand.tsv \
    ${run_prefix}_result.12.stranded.tsv
echo "Finished"; 
date